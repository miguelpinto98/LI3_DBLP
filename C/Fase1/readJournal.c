...
int verificaAuthors(char *art, int iaut) {
	int i=iaut, okAuth=1;
...
static int treatsJournal(char *art) {
	int testJournal=0;
	
	testJournal=verificaInt(art);
	if(testJournal)
		testJournal=verificaAuthors(art,testJournal);
		if(testJournal)
					...
					if(testJournal)
						...
						if(testJournal)
							...
							if(testJournal)
								...
								if(testJournal) 
									testJournal=verificaAno(art,testJournal);
									if(testJournal) {
										return testJournal;
									}
	return 0;
...
void readJournal(char *file, FILE *fr) {
	FILE *fp;
	fp = fopen(file,"r");
	char art[MAXART];
	...
	writeRejeitados(fr,file,rejeita);
	}
	fclose(fp);
}